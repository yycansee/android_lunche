...
var player_obj = {
    ...
    setPlayUrl: function (url) { /*拼接播放地址*/
        if (url.indexOf("?") == -1) {
            this.play_url = url + "?" + "stbCode=" + Util.getConfig("UserID") + "&stbModel=" + Util.getConfig("STBType");
        }
    },
    /**
     * 暂停播放操作
    ...
    play: function () { /*播放*/
        var providerID = util.getCookie(cookiesNameString.providerID);
    ...
    initMediaPlayer: function () { /*初始化播放相关环境*/
        ...
        Util.set_common_cookie("instanceId", this.instanceId);
    }
};